...
void Texture2D::CreateCubeTexture(const float *data, unsigned int width, unsigned int height, unsigned int chn)
{
    ...
    glBindTexture(targetType, textureID);
	glTexParameteri(targetType, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(targetType, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTexParameteri(targetType, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(targetType, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
