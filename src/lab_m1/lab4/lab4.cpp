...
void Lab4::RenderScene() {
	...
	modelMatrix *= transform3D::Translate(0.0f, 0.5f, 0.0f);
	modelMatrix *= transform3D::Scale(1.5f, 1.5f, 1.5f);
    modelMatrix *= transform3D::Translate(translateX, translateY, translateZ);
    RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);

    modelMatrix = glm::mat4(1);
    //modelMatrix *= transform3D::RotateOY(planetStepOY);
    modelMatrix *= transform3D::Translate(0.0f, 0.5f, -5.f);
...
void Lab4::Update(float deltaTimeSeconds)
{
    ...
    planetStepOY += 1 * deltaTimeSeconds;
...
void Lab4::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_U) {
        miniViewportArea.height += 100;
        miniViewportArea.width += 100 * 16.f / 9.f;
    }
	if (key == GLFW_KEY_O) {
        miniViewportArea.height -= 100.f;
		miniViewportArea.width -= 100.f * 16.f / 9.f;
	}
    if (key == GLFW_KEY_I) {
